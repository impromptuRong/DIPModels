...
## torchvision.models._utils.IntermediateLayerGetter(backbone, return_layers)
class ResNetFeatures(nn.Module):
    ...
    def __init__(self, architecture, pretrained=False, progress=False, 
        ...
        self.feature_block = [torchvision.models.resnet.BasicBlock, torchvision.models.resnet.Bottleneck]
...
class MobileNetFeatures(nn.Module):
    def __init__(self, architecture='mobilenet_v2', pretrained=False, progress=False, 
        ...
        self.architecture.setdefault('init_block', torchvision.models.mobilenet.ConvBNReLU)
...
#         ]
    ...
    def default_settings(architecture):
        init_block = torchvision.models.mobilenet.ConvBNReLU
        ## exact same as torchvision.models.mobilenet.InvertedResidual
...
class BackboneWithFPN(nn.Module):
    ...
    """
    def __init__(self, backbone, out_channels=None, return_layers=None, in_channels=None, extra_blocks='rcnn'):
        ...
        if in_channels is None:
            try:
                ...
                in_channels = [_['num_chs'] for _ in self.backbone.feature_info]
        ...
        if out_channels is None:
            ...
            if extra_blocks is not None:
                ...
                if extra_blocks == 'LastLevelP6P7' or extra_blocks == 'retinanet':
                    extra_blocks = torchvision.ops.feature_pyramid_network.LastLevelP6P7(self.out_channels, self.out_channels)
                # LastLevelMaxPool for faster rcnn
                elif extra_blocks == 'LastLevelMaxPool' or extra_blocks.endswith('rcnn'):
...
class SubPixelDepthwiseConv2d(SubPixelConv2d):
    def __init__(self, in_channels, out_channels, stride=2, 
        ...
        super(SubPixelDepthwiseConv2d, self).__init__(
            in_channels, out_channels, stride, (DepthwiseConv2d, args))
...
class SubPixelInvertedResidual(SubPixelConv2d):
    def __init__(self, in_channels, out_channels, stride=2, 
                 kernel_size=3, padding='default', expand_ratio=1, norm_layer='batch', 
                 activation=nn.ReLU6(inplace=True)):
        ## torchvision.models.mobilenet.InvertedResidual(inp, oup, stride, expand_ratio)
        args = {'kernel_size': kernel_size, 'expand_ratio': expand_ratio, 'padding': padding, 
                'norm_layer': norm_layer, 'activation': activation}
        super(SubPixelInvertedResidual, self).__init__(
            in_channels, out_channels, stride, (InvertedResidual, args))
...
class ResizeDepthwiseConv2d(ResizeConv2d):
    def __init__(self, in_channels, out_channels, stride=2, 
        ...
        super(ResizeDepthwiseConv2d, self).__init__(
            in_channels, out_channels, stride, (DepthwiseConv2d, args), mode=mode)
...
class ResizeInvertedResidual(ResizeConv2d):
    def __init__(self, in_channels, out_channels, stride=2, 
        ...
        super(ResizeInvertedResidual, self).__init__(
            in_channels, out_channels, stride, (InvertedResidual, args), mode=mode)
...
class MobileUNet(UNet):
    def __init__(self, num_classes, scale_factor=2, resize_mode='bilinear',
        ...
        super(MobileUNet, self).__init__(
            ...
            encoder, decoder, out_channels, return_logits, **kwargs)
    ...
    def _default_uplayer(self, block, mode='bilinear'):
        ...
        def f(in_c, out_c, stride, **kwargs):
            args = {k: v for k, v in {**default_args, **kwargs}.items()
                    if k in inspect.getargspec(block)[0]}
            return ResizeConv2d(in_c, out_c, stride, conv=(block, args), mode=mode)
            # return SubPixelConv2d(in_c, out_c, stride, conv=(block, args))
        return f
...
################################################################
## Todo: make it support MLP: layers = [1024, 1024, 1024] etc
...
class RCNN(tmdet.generalized_rcnn.GeneralizedRCNN):
    """ Build the FasterRCNN/MaskRCNN detection model.
        ...
        backbone: the backbone (include fpn)
        num_classes (None): specify the number of labels. If given, the value will overwrite config.
        masks (None): whether to include mask header. 
            None: if config has mask entries, model will add a default mask header and then overwrite 
                the header with config. if config doens't have mask entries, model will exclude mask header.
            True: model will force add a default mask header and then overwrite it with add any valid 
                mask params in config.
            False: model will force exclude mask header, even config has mask entries.
        keypoints (None): number of keypoints
            None: if config has keypoint entries, model will add a default keypoint header(19 kps) 
                and then overwrite the header with config. if config doens't have keypoint entries, 
                model will exclude keypoint header.
            >0: model will force add a default keypoint header and then overwrite it with any valid 
                keypoint params in config.
            0: model will force exclude keypoint header, even config has keypoint entries.
        config ({}): a dictionary with configs/params to build the model. 
            It can be a full RCNN.config setting, or a dictionary with only few params. Ungiven
            params will use default RCNN parameters.
        pretrained (False): the pretrained model.
            False: No pretrained weights are used
            True: will use default torchvision.models.detection models build on resnet50fpn
            str: the file path "{}.pt" for the pretrained model
        layers ("backbone+fpn+headers"): which layer to load
    ...
    """
    def __init__(self, backbone, num_classes=None, masks=None, keypoints=None, 
                 config={}, pretrained=False, layers="backbone+fpn+headers"):
        self.config = self.get_config(num_classes, masks, keypoints, config)
        ...
        self.load_pretrain(pretrained, layers)
    ...
    def get_rpn(self):
        ...
        rpn_anchor = tmdet.anchor_utils.AnchorGenerator(**rpn_params['anchor'])
    ...
    def load_pretrain(self, pretrained, layers="backbone+fpn+headers"):
        if pretrained:
    ...
    def get_config(self, num_classes, masks, keypoints, config):
...
class RCNNFULL(RCNN):
    ...
    def eager_outputs(self, losses, detections):
        ...
        # we always return losses and detections.
        if self.training:
...
class RetinaNet(tmdet.retinanet.RetinaNet):
    def __init__(self, backbone, num_classes=None, masks=None, keypoints=None, 
                 config={}, pretrained=False, layers="backbone+fpn+headers"):
        self.config = self.get_config(num_classes, masks, keypoints, config)
        # self.config['backbone'] = None
        ...
        anchor_generator, header = self.get_headers()
        super(RetinaNet, self).__init__(
            backbone, num_classes, anchor_generator=anchor_generator, head=header, 
            proposal_matcher=None, **self.config['head_params']['header']
        )
        self.transform = self.get_transform()  # overwrite transform
    ...
    def get_headers(self):
        in_channels = self.config['in_channels']
        head_params = self.config['head_params']
        
        anchor_generator = tmdet.anchor_utils.AnchorGenerator(**head_params['anchor'])
        header = tmdet.retinanet.RetinaNetHead(
            in_channels, anchor_generator.num_anchors_per_location()[0], head_params['num_classes']
        )
        
        return anchor_generator, header
    ...
    def load_pretrain(self, pretrained, layers="backbone+fpn+headers"):
        if pretrained:
            if isinstance(pretrained, str):
                ...
                m = tmdet.retinanet_resnet50_fpn(
                    pretrained=True, progress=False, pretrained_backbone=False
                )
                weights = m.state_dict()
    ...
    def get_config(self, num_classes, masks, keypoints, config):
        ...
        if num_classes is not None:
            default_config['num_classes'] = num_classes
            default_config['head_params']['num_classes'] = num_classes
...
def binary_indicators(y_true, y_pred, probs=None):
    ...
    if probs is not None:
        fpr, tpr, thresholds = metrics.roc_curve(y_true, probs, pos_label=1)
        auc = metrics.auc(fpr, tpr)
    else:
        fpr, tpr, thresholds = None, None, None
        auc = None
    
    return {'cm': cm, 'acc': acc, 'auc': auc, 
